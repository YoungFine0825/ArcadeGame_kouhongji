...
{
    // Public so it can be used publicly as a T2 for PathPlugin.
    ...
    public class Path
    {
        ...
        [SerializeField] public PathType type;
        [SerializeField] public int subdivisionsXSegment; // Subdivisions x each segment
        [SerializeField] public int subdivisions; // Stored by PathPlugin > total subdivisions for whole path (derived automatically from subdivisionsXSegment)
        [SerializeField] public Vector3[] wps; // Waypoints (modified by PathPlugin when setting relative end value and change value) - also modified by DOTweenPathInspector
        [SerializeField] public ControlPoint[] controlPoints; // Control points used by non-linear paths
        [SerializeField] public float length; // Unit length of the path
        [SerializeField] public bool isFinalized; // TRUE when the path has been finalized (either by starting the tween or if the path was created by the Path Editor)

        [SerializeField] public float[] timesTable; // Connected to lengthsTable, used for constant speed calculations
        [SerializeField] public float[] lengthsTable; // Connected to timesTable, used for constant speed calculations
        public int linearWPIndex = -1; // Waypoint towards which we're moving (only stored for linear paths, when calling GetPoint)
        ...
        public Vector3[] nonLinearDrawWps; // Used to store non-linear path gizmo points when inside Unity editor (also used by DOTweenPathInspector)
        public Vector3 targetPosition; // Set by PathPlugin at each update
        public Vector3? lookAtPosition; // Set by PathPlugin in case there's a lookAt active
        ...
        //        // (newWps length must be at least 1).
        //        // Internal so DOTweenPathInspector can use it
        //        public void AssignWaypoints(List<Vector3> newWps)
        //        {
