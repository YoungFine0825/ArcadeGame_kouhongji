...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class MediaPlayer : MonoBehaviour
	{
        ...
        /// <returns></returns>
        public bool OpenVideoFromNetAsset(string url, UnityAction<int,int> eventHandler)
        {
            if (string.IsNullOrEmpty(url))
            {
                return false;
            }
            NetAssetInfo info = NetAssetService.GetInstance().GetCachedNetAssetInfoByUrl(url);
            if (info == null)
            {
                JW.Common.Log.LogE("MediaPlayer Play Net Asset Error No Cached Asset Info :" + url);
                return false;
            }
            if (eventHandler!=null)
            {
                Events.AddListener(delegate(MediaPlayer pp, MediaPlayerEvent.EventType et, ErrorCode ec)
                {
                    if (et == MediaPlayerEvent.EventType.MetaDataReady)
                    {
                        if (m_Info != null)
                        {
                            int ww = m_Info.GetVideoWidth();
                            int hh = m_Info.GetVideoHeight();
                            float wScale = 1080f / ww;
                            float hScale = 1920f / hh;
                            float s = Mathf.Min(wScale, hScale);
                            JW.Common.Log.LogD("Video Info W:" + ww.ToString());
                            JW.Common.Log.LogD("Video Info H:" + hh.ToString());
                            //
                            gameObject.transform.localScale = new Vector3(ww / 100.0f * s, hh / 100.0f * s);
                            //
                        }
                    }
                    ...
                    if (eventHandler != null)
                    {
                        eventHandler((int)et, (int)ec);
                    }
                });
            }
            //
            OpenVideoFromFile(FileLocation.AbsolutePathOrURL, info.GetCachedFilePath(), true);
            return true;
        }
