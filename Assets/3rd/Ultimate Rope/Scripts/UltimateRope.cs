                ...
                for(int nLink = 0; nLink < rope.RopeNodes[nNode].segmentLinks.Length; nLink++)
                {
                    if(rope.RopeType != UltimateRope.ERopeType.ImportBones)
                    {
                        ...
                        if(rope.RopeNodes[nNode].bIsCoil == false)
                        {
							...
							rope.RopeNodes[nNode].segmentLinks[nLink].GetComponent<Rigidbody>().drag=1.0f;
							rope.RopeNodes[nNode].segmentLinks[nLink].GetComponent<Rigidbody>().angularDrag=1.0f;
...
public class UltimateRope : MonoBehaviour
{
    ...
    [RopePersistAttribute] public float m_fCurrentExtension = 0.0f;
                ...
                else
                {
                    ...
                    for(int nLink = 0; nLink < node.segmentLinks.Length; nLink++)
                    {
                        ...
                        if(node.segmentLinks[nLink].GetComponent<Rigidbody>() == null)
                        {
							...
							node.segmentLinks[nLink].GetComponent<Rigidbody>().drag=1.0f;
							node.segmentLinks[nLink].GetComponent<Rigidbody>().angularDrag=1.0f;
                        }
        ...
        else if(RopeType == ERopeType.ImportBones)
        {
            ...
            for(int nBone = 0; nBone < ImportedBones.Length; nBone++)
            {
                ...
                if(ImportedBones[nBone].goBone.GetComponent<Rigidbody>() == null)
                {
					...
					ImportedBones[nBone].goBone.GetComponent<Rigidbody>().drag=1.0f;
					ImportedBones[nBone].goBone.GetComponent<Rigidbody>().angularDrag=1.0f;
        ...
        for(int nNode = 0; nNode < RopeNodes.Count; nNode++)
        {
            ...
            foreach(GameObject link in node.segmentLinks)
            {
                if(link)
                {
					...
					linkRigidbody.drag=1.0f;
					linkRigidbody.angularDrag=1.0f;
...
//          Debug.Log(nTriangleIndex + " " + nLinkVertexIndexStart + " " + nJumpVertices);
...
#if UNITY_EDITOR
    //
    [ContextMenu("Do Extension +")]
    private void DoTestExtensionAdd()
    {
        DoExtendRopeLinear(Time.deltaTime);
    }
    [ContextMenu("Do Extension -")]
    private void DoTestExtensionSub()
    {
        DoExtendRopeLinear(-Time.deltaTime);
    }
#endif
    ...
    //jordenwu
    public float DoExtendRopeLinear(float fIncrement)
    {
    ...
    void CreateRopeJoints(bool bCheckIfBroken = false)
    {
        ...
        if(RopeStart != null)
        {
            if(RopeStart.GetComponent<Rigidbody>() == null)
            {
				...
				RopeStart.GetComponent<Rigidbody>().drag=1.0f;
				RopeStart.GetComponent<Rigidbody>().angularDrag=1.0f;
    ...
    void SetupJoint(ConfigurableJoint joint)
    {
      ...
      joint.zMotion = ConfigurableJointMotion.Locked;
        ...
        //Print.PrintGo("LinkJointAngularXLimit = " + LinkJointAngularXLimit, "#FF00FF");
        //Print.PrintGo("LinkJointAngularYLimit = " + LinkJointAngularYLimit, "#FF00FF");
        //Print.PrintGo("LinkJointAngularZLimit = " + LinkJointAngularZLimit, "#FF00FF");
      ...
      joint.angularZMotion = Mathf.Approximately(LinkJointAngularZLimit, 0.0f) == false ? ConfigurableJointMotion.Limited : ConfigurableJointMotion.Locked;
        ...
        jointLimit.bounciness = 10;
        
        jointLimit.limit = -LinkJointAngularXLimit;
        jointLimit.contactDistance = jointLimit.limit;
