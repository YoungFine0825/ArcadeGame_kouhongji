...
#region Transform Shortcuts
        ...
        //jordenwu
        public static Tweener DOLocalMoveXYZ(this Transform target, float endXValue, float endYValue, float endZValue, float duration, bool snapping = false)
        {
            return DOTween.To(() => target.localPosition, x => target.localPosition = x, new Vector3(endXValue, endYValue, endZValue), duration)
                .SetOptions(snapping).SetTarget(target);
        }
        ...
        public static Tweener DOLocalRotateYYZ(this Transform target, float endValueX, float endValueY,float endValueZ, float duration, RotateMode mode = RotateMode.Fast)
        {
            TweenerCore<DOQuaternion, DOVector3, QuaternionOptions> t = DOTween.To(() => target.localRotation, x => target.localRotation = x, new Vector3(endValueX,endValueY,endValueZ), duration);
            t.SetTarget(target);
            t.plugOptions.rotateMode = mode;
            return t;
        }
