...
--协议定义
local Protocol=MachineNet.Protocol
local MachineNetPackerClass = DeclareClass("MachineNetPackerClass")

function MachineNetPackerClass:ctor()
    --缓冲区
    self._netDataBuff={}
    self.Uid = 0 
end

function MachineNetPackerClass:UnInit()
...
-- 重置
-- @return 无
function MachineNetPackerClass:Reset()
...
--注册Pb协议定义到Pb
function MachineNetPackerClass:RegistePbFile(buffer)
    Pb.load(buffer)
end
...
-- @return 数据包
function MachineNetPackerClass:CreatePacket(cid,params)
    ...
    if not pbName then
        LogE("CreatePacket Error No Req PbFile Message Define Cid:"..tostring(cid))
        return false
    end
    --oneof
...
-- @param date 
-- @return 无
function MachineNetPackerClass:ParsePackets(bytes)
...
function MachineNetPackerClass:GetPBCSMsgLogicMsgKey(cmd)

    local msgKey =Protocol.Rsp[cmd]
    if not msgKey then
        LogE("------>Net Rsp No Define Cmd %d", cmd)
        return nil
    end
    return msgKey
end
